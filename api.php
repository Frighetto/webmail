...
if($_POST['action'] == "getDataSource"){
    ...
    foreach($folders as $folder){
        ...
        $mail_index_list = imap_search($mailbox_instance, $search_criteria, ST_UID);         
        $list_size = $mail_index_list == false ? 0 : sizeof($mail_index_list);
        
        for ($i = $list_size - 1; 0 <= $i; $i = $i - 1) {                         
            $mail_index = $mail_index_list[$i];            
            $header = imap_headerinfo($mailbox_instance, imap_msgno($mailbox_instance, $mail_index));           
...
if($_POST['action'] == "flag"){
    ...
    if($_POST["flag"] == 'seen'){ 
        imap_setflag_full($mailbox_instance, $_POST["ids_to_flag"], '\\Seen', ST_UID);  
    } else if($_POST["flag"] == 'unseen'){ 
        imap_clearflag_full($mailbox_instance, $_POST["ids_to_flag"], '\\Seen', ST_UID);  
    } else if($_POST["flag"] == 'flag'){ 
        imap_setflag_full($mailbox_instance, $_POST["ids_to_flag"], '\\Flagged', ST_UID);  
    } else if($_POST["flag"] == 'unflag'){ 
        imap_clearflag_full($mailbox_instance, $_POST["ids_to_flag"], '\\Flagged', ST_UID);  
    }
...
if($_POST['action'] == "send"){
    ...
    for($i = 0; $i < intval($_POST['total_images']); $i++){
        $imgfilepatch = $dir . $_POST['imgname' . $i];
        $imgfile = fopen($imgfilepatch, "w") or die("Unable to open file!");
        $b64 = substr($_POST['img' . $i], strlen("data:image/png;base64,"));
        $image_content = base64_decode($b64);
        fwrite($imgfile, $image_content);
        fclose($imgfile);
    }

    $_POST['selectedwriter'] = "mailhtml";
...
if($_POST['action'] == "read"){    
    $load_body_only = true;
    $download_uri = isset($_POST['download_uri']) ? $_POST['download_uri'] : null;
    require_once 'mailbody_loginid.php';

}

if($_POST['action'] == "move"){

    $header = imap_headerinfo($mailbox_instance, imap_msgno($mailbox_instance, $_POST["id"]));

    $from = $header->from[0];
    $from = $from->mailbox . "@" . $from->host;

    if (imap_mail_move($mailbox_instance, $_POST["id"], $_POST["movement_folder"], ST_UID)) {        
...
if($_POST['action'] == "delete"){
    ...
    if($_POST['folder'] == 'INBOX.Trash'){
        imap_setflag_full($mailbox_instance, $_POST["id"], '\\Deleted', ST_UID);  
        imap_expunge($mailbox_instance);
    } else {
        if (imap_mail_move($mailbox_instance, $_POST["id"], 'INBOX.Trash', ST_UID)) {        
            imap_expunge($mailbox_instance);
        }
    }
