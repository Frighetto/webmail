...
let dlgFolderWindow: Dialog = new Dialog();
let dlgAttachmentWindow: Dialog = new Dialog();
...
let toolbarFolder: Toolbar = new Toolbar();
...
let actionMessageID: string = null;

var foldersFullName: any = null;
var folderData: any = null;
var messageDataSourceNew: any = null;
var downloadURI: any = null;
var userName: any = null;
var userMail: any = null;

let anexo_extra: number = 0;

let send_click_time = Math.floor(Date.now() / 1000);

let flag_draft = false;

/*
import { RichTextEditor, Toolbar as txtToolbar, Link, Image, HtmlEditor, QuickToolbar } from '@syncfusion/ej2-richtexteditor';
RichTextEditor.Inject(txtToolbar, Link, Image, HtmlEditor, QuickToolbar);

let defaultRTE: RichTextEditor = new RichTextEditor({ });
defaultRTE.appendTo('#newmailtxteditor');
*/

window.addEventListener('message', event => {        
    if (event.origin === 'http://localhost:4000') {              
        let html_data = JSON.parse(event.data); 
        let mailhtml = atob(html_data['mailhtml']);        
        let keys = Object.keys(html_data);
        let total_images = keys.length - 1;
        let count = 0;
        ...
        for(let i = 0; i < keys.length; i++){
            if(keys[i] !== 'mailhtml'){
                formData.append("imgname" + count, keys[i]);
                formData.append("img" + count, html_data[keys[i]]);                
                count++;
            }
        }
                         
        formData.append('loginid', (document.getElementById('loginbutton') as HTMLElement).getAttribute("value"));        
        formData.append('to', (document.getElementById('autoTo') as HTMLInputElement).value);
        formData.append('cc', (document.getElementById('autoCc') as HTMLInputElement).value);
        formData.append('cco', (document.getElementById('autoCco') as HTMLInputElement).value);
        formData.append('subject', (document.getElementById('txtSubject') as HTMLInputElement).value);
        formData.append('mailhtml', mailhtml);        
        formData.append('send', "default");
        formData.append('action', "send");
        if(flag_draft){
            formData.append('draft', "default");
        }
        
        var myUploadedFile = (document.getElementById("attachment") as HTMLInputElement).files[0];        
        if(myUploadedFile !== undefined){ 
            formData.append(myUploadedFile.name, myUploadedFile);
        }
        for(let i = 0; i < anexo_extra; i++){ 
            myUploadedFile = (document.getElementById("attachment" + i) as HTMLInputElement).files[0];        
            if(myUploadedFile !== undefined){ 
                formData.append(myUploadedFile.name, myUploadedFile);
            }
        }        
...
window.home = (): void => {   
    ...
    http_request.onload = function(e) {
        ...
        downloadURI = downloadURI.substr(0, downloadURI.indexOf("api.php")) + "download_loginid.php";        
        renderMainSection();
        ...
        renderMailDialogs();
        renderFolderDialogs();
        renderAttachmentsDialogs();
    }
    let loginid = (document.getElementById('loginbutton') as HTMLElement).getAttribute("value");    
    var params = 'loginid=' + encodeURIComponent(loginid);    
    params = params + '&action=' + encodeURIComponent("getDataSource"); 
    http_request.send(params);   
    
    let logout_url = 'http://' + ip + '/webmail/sair.php';  
    (document.getElementById('logout') as HTMLElement).setAttribute("onclick", "parent.document.location='" + logout_url + "'");

    document.getElementById('diretoriosopt').onclick = showFolderDialog;
    ...
    */
    let ajaxHTML: Ajax;    
    ajaxHTML = new Ajax('src/home/newmail.html', 'GET', true);
...
function renderMoveToList(): void {   
    ...
    for(let i = 0; i < folderData.length; i = i + 1){        
...
function moveToSelect(args: DropDownSelectEventArgs): void {        
    ...
    http_request.onload = function(e) {        
        if (args.itemData.text) {
            ...
            let key2: string = 'ContactID';
            selectedMessage[key] = args.itemData.text;
            selectedMessage[key2] = http_request.response;
...
export function showEmptyMessage(): void {
    ...
    document.getElementById('attachmentDialog').innerHTML = null;
    anexo_extra = 0;
    renderAttachmentsDialogs();
}
...
function setReadStyleMessage(contactID: string, readStyle: string): void {
    ...
    if (data !== null) {
        ...
        if (readStyle === 'Read') {            
            ...
            (selectedListElement.getElementsByClassName('template-container')[0] as HTMLElement).className = 'template-container';
            ...
            (selectedListElement.getElementsByClassName('subjectstyle')[0] as HTMLElement).className = 'subjectstyle Unread';
            (selectedListElement.getElementsByClassName('template-container')[0] as HTMLElement).className = 'template-container Unread-parent';
            setReadCount('Read');
            newFlag = "unseen";
        }
        var ip = location.host;    
...
function getSelectedMessage(): { [key: string]: Object } {
    if (grpListObj.getSelectedItems()) {
        ...
        let key2: string = 'Folder';
        for (let i: number = 0; i < messageDataSource.length; i++) {
            if (messageDataSource[i][key].toString() === selectedData[key].toString() && messageDataSource[i][key2].toString() === selectedData[key2].toString()) {
    ...
    setTimeout(() => {
        ...
        if(data['ContactID'].toString() !== actionMessageID){
            ...
            http_request.onload = function(e) {                      
                ...
                if(selectedFolderName == "Drafts"){                     
                    showNewMailPopup("Draft");
                } else {                                         
                    let key: string = 'ReadStyle';
            ...
            params += '&download_uri=' + encodeURIComponent(downloadURI);    
...
function autoSearchSelect(args: ChangeEventArgs): void {       
    if (args.value) {        
        let dataSource: { [key: string]: Object }[] = messageDataSource;//getFilteredDataSource(messageDataSource, 'Folder', selectedFolderName);         
        grpListObj.dataSource = getFilteredDataSource(dataSource, 'ContactTitle', args.value.toString());
...
function renderFolderDialogs(): void{
    dlgFolderWindow = new Dialog({
        width: '80%',
        height: '93%',
        target: document.body,
        animationSettings: { effect: 'None' },
    ...
    dlgFolderWindow.appendTo('#folderDialog');
    dlgFolderWindow.hide();
        
    document.getElementById('folderDialog_dialog-content').innerHTML = '<h3>Diretórios</h3><div style="right:20px;position: absolute; top: 35px"><button id="diretorioclose" class="e-control e-btn e-lib btn-shadow-hide e-icon-btn"><span class="e-btn-icon ej-icon-Close"></span></button></div>';    
    let html_folder_options = '';
    for(let i = 0; i < folderData.length; i = i + 1){
        html_folder_options += '<option>' + foldersFullName[folderData[i].Name] + '</option>';
    }    
    document.getElementById('folderDialog_dialog-content').innerHTML += '<div><span style="font-size: 20px">Diretório Superior:</span><select id="diretoriosuperior" style="width: 200px; height: 30px">' + html_folder_options + '</select><input type="text" id="novodiretorio" placeholder="Novo Diretório" style="width: 200px" class="e-input"><a id="criarnovodiretorio">Criar Novo Diretório</a></div>';

    document.getElementById('folderDialog_dialog-content').innerHTML += '<br><div><span style="font-size: 20px">Diretório Renomeado:</span><select id="diretoriorenomeado" style="width: 200px; height: 30px">' + html_folder_options + '</select><input type="text" id="novonomediretorio" placeholder="Novo Nome do Diretório" style="width: 200px" class="e-input"><a id="renomeardiretorio">Renomear Diretório</a></div>';

    document.getElementById('folderDialog_dialog-content').innerHTML += '<br><div><select id="diretorioexcluido" style="width: 200px; height: 30px">' + html_folder_options + '</select><a id="excluirdiretorio">Excluir</a></div>';
    
    document.getElementById('diretorioclose').onclick = btnCloseFolders;
    document.getElementById('criarnovodiretorio').onclick = criarNovoDiretorio;
    document.getElementById('renomeardiretorio').onclick = renomearDiretorio;
    document.getElementById('excluirdiretorio').onclick = exluirDiretorio;
    
}

function add_anexo(): void {        
    document.getElementById('attachment_input').innerHTML += '<br><input id="attachment' + anexo_extra + '" type="file" />';
    anexo_extra += 1;
}

function renderAttachmentsDialogs(): void{
    dlgAttachmentWindow = new Dialog({
        width: '80%',
        height: '93%',
        target: document.body,
        animationSettings: { effect: 'None' },
    ...
    dlgAttachmentWindow.appendTo('#attachmentDialog');
    dlgAttachmentWindow.hide();
    
    document.getElementById('attachmentDialog_dialog-content').innerHTML = '<h3>Anexos</h3><div style="right:20px;position: absolute; top: 35px"><button id="attachmentclose" class="e-control e-btn e-lib btn-shadow-hide e-icon-btn"><span class="e-btn-icon ej-icon-Close"></span></button></div>';    
        
    document.getElementById('attachmentDialog_dialog-content').innerHTML += '<br><input id="attachment" type="file" />';
    document.getElementById('attachmentDialog_dialog-content').innerHTML += '<div id="attachment_input"></div>';
    document.getElementById('attachmentDialog_dialog-content').innerHTML += '<br><button id="add_anexo">+ Anexo</button>';

    document.getElementById('attachmentclose').onclick = btnCloseAttachments;
    document.getElementById('add_anexo').onclick = add_anexo;
    
}

function exluirDiretorio(): void{       
    var ip = location.host;    
    if(ip.indexOf(":") !== -1){
        ip = ip.split(":")[0];
    }        
    ...
    http_request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		
    http_request.onload = function(e) {        
        if(http_request.response === "deleted"){ 
            alert("Diretório excluído, atualize a página");
        } else { 
            alert("Erro: " + http_request.response);
        }
    }
    let loginid = (document.getElementById('loginbutton') as HTMLElement).getAttribute("value");    
    var params = 'loginid=' + encodeURIComponent(loginid);    
    params = params + '&action=' + encodeURIComponent("deletefolder"); 
    let removed_folder = (document.getElementById('diretorioexcluido') as HTMLInputElement).value;    
    params = params + '&removed_folder=' + encodeURIComponent(removed_folder); 
    http_request.send(params);   
}

function criarNovoDiretorio(): void{    
    var ip = location.host;    
    if(ip.indexOf(":") !== -1){
        ip = ip.split(":")[0];
    }        
    ...
    http_request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		
    http_request.onload = function(e) {       
        if(http_request.response === "created"){ 
            alert("Novo diretório criado, atualize a página");
        } else { 
            alert("Erro: " + http_request.response);
        }
    }
    let loginid = (document.getElementById('loginbutton') as HTMLElement).getAttribute("value");    
    var params = 'loginid=' + encodeURIComponent(loginid);    
    params = params + '&action=' + encodeURIComponent("createfolder"); 
    let novodiretorio = (document.getElementById('novodiretorio') as HTMLInputElement).value;    
    params = params + '&add_folder=' + encodeURIComponent(novodiretorio); 
    let parent_folder = (document.getElementById('diretoriosuperior') as HTMLInputElement).value;    
    params = params + '&parent_folder=' + encodeURIComponent(parent_folder); 
    http_request.send(params);   
}

function renomearDiretorio(): void{    
    var ip = location.host;    
    if(ip.indexOf(":") !== -1){
        ip = ip.split(":")[0];
    }        
    ...
    http_request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');		
    http_request.onload = function(e) {       
        if(http_request.response === "renamed"){ 
            alert("Diretório renomeado, atualize a página");
        } else { 
            alert("Erro: " + http_request.response);
        }
    }
    let loginid = (document.getElementById('loginbutton') as HTMLElement).getAttribute("value");    
    var params = 'loginid=' + encodeURIComponent(loginid);    
    params = params + '&action=' + encodeURIComponent("rename_folder"); 
    let renamed_folder = (document.getElementById('diretoriorenomeado') as HTMLInputElement).value;    
    params = params + '&renamed_folder=' + encodeURIComponent(renamed_folder); 
    let new_folder_name = (document.getElementById('novonomediretorio') as HTMLInputElement).value;    
    params = params + '&new_folder_name=' + encodeURIComponent(new_folder_name); 
    http_request.send(params);   
}

function btnCloseFolders(): void{
    dlgFolderWindow.hide();
}

function btnCloseAttachments(): void{
    dlgAttachmentWindow.hide();
}
...
function flagRequest(url: string, loginid: string, folder: string, ids_to_flag: string, flag: string){    
    var http_request = new XMLHttpRequest();			
    http_request.open("POST", url, true);	       
    http_request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    http_request.onload = function(e) {  
...
function toolbarClick(args: ClickEventArgs): void {
    if (args.item) {
        if (args.item.prefixIcon === 'ej-icon-Menu tb-icons') {
            ...
            var url = 'http://' + ip + '/webmail/api.php';
            let loginid = (document.getElementById('loginbutton') as HTMLElement).getAttribute("value");             
            let folder = foldersFullName[selectedFolderName];
            let ids_to_flag = "";
            if(selectedFolderName === ""){
                var folders: string[][] = [];
                var folders_names: string[] = [];
                let key: string = 'Folder';
                for (let i: number = 0; i < messageDataSource.length; i++) {
                    let folder = messageDataSource[i][key].toString();
                    if(folders_names.indexOf(folder) === -1){
                        folders_names.push(folder);
                        folders[folders_names.indexOf(folder)] = [];
                    }                    
                    folders[folders_names.indexOf(folder)].push(messageDataSource[i]["ContactID"].toString());                    
                }
                for (let i: number = 0; i < folders_names.length; i++) {
                    let folder = folders_names[i];
                    let folder_index = folders_names.indexOf(folder);     
                    let ids_to_flag = "";               
                    for (let j: number = 0; j < folders[folder_index].length; j++) {                        
                        if(ids_to_flag !== ""){
                            ids_to_flag = ids_to_flag + ",";
                        }                            
                        ids_to_flag = ids_to_flag + folders[folder_index][j];                        
                    }
                    flagRequest(url, loginid, foldersFullName[folder], ids_to_flag, "seen");
                }                
            } else {                
                let key: string = 'Folder';
                for (let i: number = 0; i < messageDataSource.length; i++) {
                    if (messageDataSource[i][key].toString() === selectedFolderName) {
                        if(ids_to_flag !== ""){
                            ids_to_flag = ids_to_flag + ",";
                        }                            
                        ids_to_flag = ids_to_flag + messageDataSource[i]["ContactID"];
                    }
                }
            ...
            let movement_folder = args.item.text === 'Junk' ? 'Junk' : 'Trash';
            let selectedMessage: { [key: string]: Object } = getSelectedMessage();
...
export function loadmessage(): void {
    let selectedMessage: { [key: string]: Object } = getSelectedMessage();
    let iframe: HTMLIFrameElement = (document.getElementById('messageiframe') as HTMLIFrameElement); 
    let key = 'Message';
    iframe.contentWindow.postMessage('messageContent:' + selectedMessage[key].toString(), '*');
}
...
export function DraftClick(): void { 
    flag_draft = true;
    if((Math.floor(Date.now() / 1000) - send_click_time) > 10){      
        send_click_time = Math.floor(Date.now() / 1000);        
        let iframe: HTMLIFrameElement = (document.getElementById('messageiframe') as HTMLIFrameElement);    
        iframe.contentWindow.postMessage('messageSender:' + userName, '*');        
    }
}
...
export function sendClick(): void {        
    ...
    /*    
    Nao foi possivel implementar o rich text editor no mesmo projeto pois havia incompatibilidades nas dependencias
    nao foi possivel obter a mensagem diretamente do iframe por causa de cross brower origin, 
    mesmo ja configurado no webpack.config.js do projeto do rich text pra liberar pra todas as origens
    ...
    params += '&to=' + encodeURIComponent((document.getElementById('autoTo') as HTMLInputElement).value);
    params += '&cc=' + encodeURIComponent((document.getElementById('autoCc') as HTMLInputElement).value);
    params += '&subject=' + encodeURIComponent((document.getElementById('txtSubject') as HTMLInputElement).value);    
    //let sendMailContentMessage: HTMLElement = document.getElementById('mailContentMessage') as HTMLElement;
    let iframe: HTMLIFrameElement = (document.getElementById('messageiframe') as HTMLIFrameElement);
    let sendMailContentMessage: HTMLElement = iframe.contentDocument.getElementById("defaultRTE_rte-edit-view");
...
export function attachmentButtonClick(): void {    
    dlgAttachmentWindow.show();    
}
        ...
        if (target.className.indexOf('header-right-pane') !== -1) {
            ...
            if (target.tagName === 'SPAN' || (target.children && target.children.length > 0)) {
                ...
                if (target.className === 'e-btn-icon ej-icon-Favorite ' + selectedFolderName) {
                    ...
                    let selectedMessage: { [key: string]: Object } = getSelectedMessage();
                    actionMessageID = selectedMessage['ContactID'].toString();
                    if (target.className.indexOf('ej-icon-Delete') !== -1) {                        
                        ...
                        if (parentNode.title === 'Mark as read') {
                            ...
                            key = 'ContactID';
                            actionMessageID = selectedMessage[key].toString();
                            setReadStyleMessage(selectedMessage[key].toString(), 'Read');
...
function newmailWindowItemClick(): void {
    if (selectedToolbarItem) {
        if (selectedToolbarItem === 'tb-item-window-mail') {
            ...
            discardButtonClick();
        } else if (selectedToolbarItem === 'Attach') {
            attachmentButtonClick();
        } else if (selectedToolbarItem === 'Draft') {
            DraftClick();
        }
    }
