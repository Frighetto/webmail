define(["require", "exports", "@syncfusion/ej2-navigations", "@syncfusion/ej2-lists", "@syncfusion/ej2-buttons", "@syncfusion/ej2-dropdowns", "@syncfusion/ej2-popups", "@syncfusion/ej2-base", "@syncfusion/ej2-layouts", "./datasource", "./newmail", "./readingpane"], function (require, exports, ej2_navigations_1, ej2_lists_1, ej2_buttons_1, ej2_dropdowns_1, ej2_popups_1, ej2_base_1, ej2_layouts_1, datasource_1, newmail_1, readingpane_1) {
    ...
    var dlgFolderWindow = new ej2_popups_1.Dialog();
    var dlgAttachmentWindow = new ej2_popups_1.Dialog();
    ...
    var toolbarFolder = new ej2_navigations_1.Toolbar();
    ...
    var userMail = null;
    var anexo_extra = 0;
    var send_click_time = Math.floor(Date.now() / 1000);
    var flag_draft = false;
    window.addEventListener('message', function (event) {
        if (event.origin === 'http://localhost:4000') {
            var html_data = JSON.parse(event.data);
            var mailhtml = atob(html_data['mailhtml']);
            var keys = Object.keys(html_data);
            var total_images = keys.length - 1;
            var count = 0;
            ...
            formData.append('loginid', document.getElementById('loginbutton').getAttribute("value"));
            formData.append('to', document.getElementById('autoTo').value);
            formData.append('cc', document.getElementById('autoCc').value);
            formData.append('cco', document.getElementById('autoCco').value);
            formData.append('subject', document.getElementById('txtSubject').value);
            ...
            var myUploadedFile = document.getElementById("attachment").files[0];
            if (myUploadedFile !== undefined) {
                formData.append(myUploadedFile.name, myUploadedFile);
            }
            for (var i = 0; i < anexo_extra; i++) {
                myUploadedFile = document.getElementById("attachment" + i).files[0];
                if (myUploadedFile !== undefined) {
                    formData.append(myUploadedFile.name, myUploadedFile);
                }
            }
    ...
    function renderFolderDialogs() {
        dlgFolderWindow = new ej2_popups_1.Dialog({
            width: '80%',
            height: '93%',
            target: document.body,
            animationSettings: { effect: 'None' },
    ...
    function renderAttachmentsDialogs() {
        dlgAttachmentWindow = new ej2_popups_1.Dialog({
            width: '80%',
            height: '93%',
            target: document.body,
            animationSettings: { effect: 'None' },
    ...
    function exluirDiretorio() {
        ...
        var removed_folder = document.getElementById('diretorioexcluido').value;
        params = params + '&removed_folder=' + encodeURIComponent(removed_folder);
        http_request.send(params);
    }
    function criarNovoDiretorio() {
        var ip = location.host;
        if (ip.indexOf(":") !== -1) {
            ip = ip.split(":")[0];
        }
        ...
        var novodiretorio = document.getElementById('novodiretorio').value;
        params = params + '&add_folder=' + encodeURIComponent(novodiretorio);
        var parent_folder = document.getElementById('diretoriosuperior').value;
        params = params + '&parent_folder=' + encodeURIComponent(parent_folder);
        http_request.send(params);
    }
    function renomearDiretorio() {
        var ip = location.host;
        if (ip.indexOf(":") !== -1) {
            ip = ip.split(":")[0];
        }
        ...
        var renamed_folder = document.getElementById('diretoriorenomeado').value;
        params = params + '&renamed_folder=' + encodeURIComponent(renamed_folder);
        var new_folder_name = document.getElementById('novonomediretorio').value;
        params = params + '&new_folder_name=' + encodeURIComponent(new_folder_name);
        http_request.send(params);
    }
    function btnCloseFolders() {
        dlgFolderWindow.hide();
    }
    function btnCloseAttachments() {
        dlgAttachmentWindow.hide();
    }
    ...
    exports.attachmentButtonClick = attachmentButtonClick;
    ...
    function newmailWindowItemClick() {
        if (newmail_1.selectedToolbarItem) {
            ...
            else if (newmail_1.selectedToolbarItem === 'Attach') {
                attachmentButtonClick();
            }
            else if (newmail_1.selectedToolbarItem === 'Draft') {
                DraftClick();
            }
        }
